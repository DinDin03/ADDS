...
void Heap<T>::insert(T value) {
  values.push_back(value); // add value to the end
...
void Heap<T>::remove(T value) {
  ...
  // Swap the value with the last element and remove the last element
...
template <typename T>
T Heap<T>::getMin() {
  if (values.empty()) throw std::out_of_range("Heap is empty"); // Handle empty heap case
  return values[0]; // Return the smallest element
}
